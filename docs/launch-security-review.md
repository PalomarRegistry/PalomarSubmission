...
4. Fetch and replay Mathlib's authenticated cache from the official Mathlib
...
5. Compile `Challenge.lean` directly with trusted Lean against only frozen
   allowlisted output. Snapshot its `Challenge.olean` and audit Lean's source
   dependency list before candidate Lake configuration executes.
...
- Palomar-indexed dependencies are representable in registry metadata but are
  not executable Challenge inputs until their statement surfaces can be
